...
# --- Tesseract Path ---
# Ensure this path matches your local installation
...
def read_plate_text(plate_img):
    ...
    # Corrected string formatting for Tesseract config
...
def main():
    ...
    while True:
        ...
        if candidates:
            ...
            # Annotate extracted text
            if plate_text:
                x = int(np.max(box[:, 0]))
                y = int(np.max(box[:, 1])) + 25
                
                # Keep text inside frame boundaries
